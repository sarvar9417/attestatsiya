...
export default function PhraseProgress({ stats, totalLearned, totalPhrases, dueCount, streak }: Props) {
  const { t } = useI18n()
  return (
    <div className="card p-3">
      ...
      <div className="flex items-center justify-between mt-3 pt-2 border-t border-gray-100 dark:border-gray-700 text-center">
        <div>
          ...
          <p className="text-[9px] text-gray-400">{t('phraseProgress.totalPhrases', { total: totalPhrases.toLocaleString() })}</p>
        </div>
        <div>
          <p className="text-sm font-bold text-orange-600">{dueCount}</p>
          <p className="text-[9px] text-gray-400">{t('phraseProgress.review')}</p>
        </div>
        <div className="flex items-center gap-1">
          <Flame size={14} className="text-orange-500" />
          <p className="text-sm font-bold text-gray-900 dark:text-gray-100">{streak}</p>
          <p className="text-[9px] text-gray-400">{t('phraseProgress.days')}</p>
        </div>
      </div>
    </div>
  )
